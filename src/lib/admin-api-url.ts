 ...
 * Priority order:
 * 1. NEXT_PUBLIC_ADMIN_API_URL (main variable - should be set to https://admin.onestepfitness.sg)
 * 2. NEXT_PUBLIC_API_URL (fallback)
 * 3. https://admin.onestepfitness.sg (production default)
 ...
 * 
 * Value should be: https://admin.onestepfitness.sg
 */
export function getAdminApiUrl(): string {
  ...
  if (!adminApiUrl && process.env.NEXT_PUBLIC_API_URL) {
    ...
    if (!apiUrl.includes('3000') && !apiUrl.includes('onestepfitness.sg') || apiUrl.includes('admin')) {
      adminApiUrl = apiUrl
    }
  }
  ...
  if (!adminApiUrl) {
    ...
    if (process.env.NODE_ENV === 'development') {
      ...
      if (adminApiUrl.includes('localhost') || adminApiUrl.includes('3000') || adminApiUrl.includes('3001')) {
        ...
        console.error('[AdminApiUrl] Please set NEXT_PUBLIC_ADMIN_API_URL=https://admin.onestepfitness.sg in Vercel environment')
        adminApiUrl = 'https://admin.onestepfitness.sg'
      }
    }
  }

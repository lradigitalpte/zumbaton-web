      ...
      footer={
        <div className="flex items-center justify-between w-full">
          ...
          <div className="text-right">
            {!isBookingWindowOpen && (
              <p className="text-xs text-red-600 dark:text-red-400 mb-2">This class starts within 24 hours. Booking is available from 08:00 to 22:00 SGT.</p>
            )}
            <button
              onClick={onBookClick}
              disabled={isFull || isBooking || !isBookingWindowOpen}
              title={!isBookingWindowOpen ? 'Classes starting within 24 hours can be booked only from 08:00–22:00 SGT' : undefined}
              ...
              <div className="flex items-center gap-2 mt-2">
                <div className="flex -space-x-2 shrink-0">
                  {(classItem.instructors?.length ? classItem.instructors : [{ name: classItem.instructor_name || "TBA", avatar: null, initials: (classItem.instructor_name || "??").split(" ").map((n: string) => n[0]).join("").toUpperCase().slice(0, 2) }]).slice(0, 2).map((instructor: any, idx: number) => (
                    <div key={instructor.id || idx} className="w-10 h-10 rounded-full flex items-center justify-center text-xs font-semibold text-white bg-primary overflow-hidden border-2 border-white dark:border-gray-800 shadow-sm" title={instructor.name}>
                      {instructor.avatar ? (
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-3 p-3 bg-gray-50 dark:bg-gray-800/50 rounded-lg">
              ...
              <div className="min-w-0">
                <p className="text-xs text-body-color dark:text-gray-400">Instructor{classItem.instructors?.length > 1 ? "s" : ""}</p>
                <p className="font-medium text-dark dark:text-white">{classItem.instructors?.length ? classItem.instructors.map((i: any) => i.name).join(", ") : classItem.instructor_name || "TBA"}</p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3 p-3 bg-gray-50 dark:bg-gray-800/50 rounded-lg">
              ...
              <div>
                ...
                <p className="font-medium text-dark dark:text-white">
                  {classItem.booked_count ?? 0} / {classItem.capacity} booked
                </p>
              </div>
            </div>
          </div>

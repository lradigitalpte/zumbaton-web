...
interface Class extends PublicClass {
...
interface TrialBookingClientProps {
...
export default function TrialBookingClient({
  initialClasses = [],
}: TrialBookingClientProps) {
  ...
  const [dateFilter, setDateFilter] = useState<string>(() => formatYmdSingapore());
  ...
  useEffect(() => {
    const fetchClasses = async () => {
      ...
      try {
        const params = new URLSearchParams();
        const anchorYmd = dateFilter || formatYmdSingapore();
        ...
        const result = await response.json();
        if (result.success && result.data) {
          const availableClasses = result.data.map((cls: Class) => ({ ...cls, booked_count: cls.booked_count || 0 }));
